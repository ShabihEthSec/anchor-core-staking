...
describe("anchor-core-staking", () => {
  ...
  it("Claim rewards without unstaking", async () => {
    ...
    const rewardsBalance = (
      await provider.connection.getTokenAccountBalance(userRewardsAta)
    ).value.uiAmount;

    console.log("User rewards balance after claim", rewardsBalance);

    if (rewardsBalance !== TIME_TRAVEL_IN_DAYS) {
      throw new Error(
        `Expected ${TIME_TRAVEL_IN_DAYS} rewards after claim, got ${rewardsBalance}`
      );
    }
  });

  it("Unstake right after claiming rewards", async () => {
    // Get the user rewards ATA account
    const userRewardsAta = getAssociatedTokenAddressSync(
      rewardsMint,
      provider.wallet.publicKey,
    ...
    const afterUnstakeBalance = (
      await provider.connection.getTokenAccountBalance(userRewardsAta)
    ).value.uiAmount;

    console.log("User rewards balance after unstake", afterUnstakeBalance);

    if (afterUnstakeBalance !== beforeUnstakeBalance) {
      throw new Error(
        `Expected no extra rewards immediately after claim; before=${beforeUnstakeBalance}, after=${afterUnstakeBalance}`
      );
    }
  });
});
